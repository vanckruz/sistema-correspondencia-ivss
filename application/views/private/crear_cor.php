...
#Ciclo para volcar las grupos en el combo-box
foreach($grupos as $grupo){
	//$numcontrol[]=$grupo->siglas_grupo;
	  foreach($dependencias->ivss_model->ver_grupo_individual($grupo->id_grupo)->result() as $grupos_individual){
	  	$numcontrol[]=$grupos_individual->siglas_grupo;
	  }
}



#Ciclo para obtener el grupo individual y sacar las siglas y el id de la dirección general.
foreach($dependencias->ivss_model->ver_grupo_individual($id_del_grupo)->result() as $migrupo){
	$nombre_division	= $migrupo->nombre_grupo;
	$siglas_mi_grupo	= $migrupo->siglas_grupo;
	$id_dir_gen_grupal	= $migrupo->id_dir_general;
}



#Ciclo para obtener y guardar el id de las direcciones internas.
foreach($dependencias->ivss_model->ver_dependencias_dir($id_del_grupo)->result() as $midir){
	 $id_dir=$midir->id_dir;
}



#ciclo para obtener y guardar el nombre de las direcciones y sus siglas.
foreach($dependencias->ivss_model->ver_direccion_especifica($id_dir)->result() as $mi_direccion_en_grupo){
	$siglas_dir=$mi_direccion_en_grupo->siglas_dir;
	$nombre_dir=$mi_direccion_en_grupo->nombre;
}



#Ciclo para obtener el nombre de la direccion general,las siglas y el id.
foreach($dependencias->ivss_model->ver_direccion_general_especifica($id_dir_gen_grupal)->result() as $get_id_dir) {
	$id_dir_general				= $get_id_dir->id_dir_general;
	$nombre_dir_general 		= $get_id_dir->nombre_direccion_general;
	$siglas_dir_general			= $get_id_dir->siglas_direccion_general;
	$correlativo_dir_general 	= $get_id_dir->correlativo_dir_general;
} 
...
<div id="formIngreso" style="width:950px;height:630px;margin-top:21px;border-radius:0px;">
<div class="row" style="border-bottom:1px dotted gray;">
	<div class="col-xs-12">
		<p style="font-size:1.4em">Nuevo registro de correspondencia</p>
	</div>
</div>	
	<form action="<?=$this->config->base_url();?>index.php/crearCorrespondencia" method="post" name="" id="CrearCorrespondencia" role="form" enctype="multipart/form-data">
<div class="row" style="">
...
<input type="hidden" value="<?=$id_del_grupo?>" name="id_del_grupo">
...
<div class="row">
	<div class="col-xs-6">
		...
		<div class="inner-addon left-addon">
				...
				<input type="text" class="form-control" name="dir_origen" placeholder="Dirección de salida" value="<?php echo $siglas_dir_general." - ".$nombre_dir_general ?>" readonly>
		</div>				
	</div>
...
<div class="row">
	<div class="col-xs-5">
		...
		<textarea class="form-control cont" style="width:400px;height:85px;margin-right:25px;resize:none;" name="descripcion">
