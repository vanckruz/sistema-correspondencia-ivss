...
<ul class="nav nav-tabs" role="tablist">
  ...
  <li><a href="#agregar_du" role="tab" data-toggle="tab">Agregar <span class="glyphicon glyphicon-plus-sign"></span></a></li>
</ul>
...
<div id="ultimos_asignados" style="margin-top:16px;font-family:Arial">
	<table class="table table-hover table-striped display" id="tablero_asignados">
		...
		<tbody>
<?php 
#Ciclo para ver los oficios asignados a la dirección interna correspondiente.
foreach($this->ivss_model->asignaciones_direccion_interna($id_this_direccion)->result() as $asignacion){
...
#Ciclo para ver los oficios asignados a la dirección interna correspondencia.
?>
		</tbody>
	</table>
</div>
...
<div id="vaciar_a_papelera" style="display:none;background:rgba(0,0,0,1,0.5);">
	<div style='margin:auto;margin-top:7px;width:300px;background:#f1f1f1;padding:21px;'>
		<form action="<?=$this->config->base_url()?>index.php/director/del_messages" method="post">
...
<!--**************************************************************************-->
		<form action="<?=$this->config->base_url();?>index.php/director/asignaciones" id="form_asignar_asuario" method="post">
...
<td>Divisiones Internas</td>
	...
	<td>
		<div class="onoffswitch">
  		<input type="checkbox" class="onoffswitch-checkbox" id="sel_division_interna" checked="checked" />
...
<td>
	<select name="usuario[]" id="select_users" class="" multiple="multiple" style="width:200px;float:left;margin-right:25px;border-radius:0px;"  disabled="disabled">	
	<?php
	$num_u_dir=0;
...
foreach($CI->ivss_model->ver_divisiones_direcciones($id_this_direccion)->result() as $u_d){

	foreach($CI->ivss_model->usuarios_grupos($u_d->id_grupo)->result() as $user) { 
	  echo "<tr>";
		echo "<td><b>".$user->usuario."</b></td>";
		echo "<td><div style='color:black;font-size:2em;'>".$CI->ivss_model->ver_oficios_asignados_usuarioldap($user->usuario)->num_rows()."</div></td>";
		echo "<td id='ver_oficios_asignados_a_este_usuario'>";	
		
		foreach($CI->ivss_model->ver_oficios_asignados_usuarioldap($user->usuario)->result() as $user_asignacion){ 
			echo "<div style='width:150px;height:25px;border-radius:5px;border:1px solid #EEE;background.#f1f1f1;padding:3px;color:blue;'>".$user_asignacion->num_control."</div>";
		}	
		echo "</td></tr>";
	}
...
<?php
foreach($this->ivss_model->ver_divisiones_direcciones($id_this_direccion)->result() as $div){
...
///////////////////////////////////////////////////////////////////////
foreach($this->ivss_model->ver_divisiones_direcciones($id_this_direccion)->result() as $sub_div){
	...
	foreach($this->ivss_model->ver_grupo_individual($sub_div->id_grupo)->result() as $midiv){
		...
		foreach($this->ivss_model->ver_oficios_grupales($midiv->id_grupo)->result() as $ofc_div){
			echo "<b>".$ofc_div->num_control."</b>, ";
		}
		echo "</td>";
		echo "<td>".$this->ivss_model->ver_oficios_grupales($midiv->id_grupo)->num_rows()."</td></tr>";
	}
...
<div id="agregar_du" class="tab-pane">
		...
		<div id="nuevoGrupo" class="" style="background:linear-gradient(rgba(7, 70, 217, 0.96), rgba(12, 172, 240, 0.96));color:#f1f1f1;padding:15px;font-size:1.3em;cursor:pointer;border-bottom:1px solid #f1f1f1;"><span class="caret" style="font-size:1em;"></span> Nueva División Interna</div>
			<div id="grupo" style="background: #234181 linear-gradient(#639ACA,#6095C4 20%,#3368A0 60%,#234181 100%);padding:10px;border:1px dotted gray;display:none;">
				<div class="centro" style="margin:auto;width:500px;background-color:#f9f9f9;">
					<form action="director/crearDivision" method="post" id="form_crear_dependencia">
						...
						<input type="hidden" name="direccion_dependencia" id="direccion_dependencia" class="form-control chosen" value="<?=$id_this_direccion?>">
						<input type="hidden" name="direccion_generalicima" id="direccion_generalicima" class="form-control chosen" value="<?=$id_general_dir?>">
...
<?php /* ?>
				<div id="new_users" style="background:linear-gradient(rgba(7, 70, 217, 0.96), rgba(12, 172, 240, 0.96));color:#f1f1f1;padding:15px;font-size:1.3em;cursor:pointer;border-bottom:1px solid #f1f1f1;"><span class="caret" style="font-size:1em;"></span> Nuevos usuarios directos de la <?=$nombre_direccion; ?></div>
			<div id="users" style="background: #234181 linear-gradient(#639ACA,#6095C4 20%,#3368A0 60%,#234181 100%);padding:10px;border:1px dotted gray;display:none;">
				<div class="centro" style="margin:auto;width:500px;background-color:#f9f9f9;">
				<form action="director/crear_usuarios" id="crear_usuarios" name="crear_usuarios" method="post">
					<input type="hidden" value="<?=$id_this_direccion;?>" name="id_de_la_direccion_interna">
...
<script>
  ...
  $(this).tab('show')
	...
	$(document).on("ready",function(evento){
		...
		//$("#select_users").chosen({allow_single_deselect: true,width:"300px"});	
...
$("#status").on("change",function(){
		$.post("<?=$this->config->base_url()?>index.php/director/actualizar_status",$("#form_status").serialize(),function(){
...
$.post("<?=$this->config->base_url();?>index.php/director/mensajes/"+id_this+"",function(data){
...
function enviarMensaje(){
	...
	if(campoMensaje !=""){
		$.ajax({
		type:"post",
		url:"<?=$this->config->base_url();?>index.php/director/crearMensaje",
...
#estatus_procesado{background-color: #00a300;color:white;width:100px;height:100px;text-align:center;padding-top:35px;float: right;}
...
#estatus_creado_group{background-color:#D0D0D0;width:125px;height:33px;text-align:center;padding:7px 10px 7px 3px;}
#estatus_rojo_group{background-color:#ee1111;color:white;width:125px;height:33px;text-align:center;padding:7px 10px 7px 3px;}
#estatus_purpura_group{background-color:purple;color:#f1f1f1;width:125px;height:33px;text-align:center;padding:7px 10px 7px 3px;}
#estatus_amarillo_group{background-color:#ffc40d;width:125px;height:33px;text-align:center;padding:7px 10px 7px 3px;}
#estatus_verde_group{background-color: #00a300;color:#f1f1f1;width:125px;height:33px;text-align:center;padding:7px 10px 7px 3px;}
#estatus_default_group{background:linear-gradient(rgba(7, 70, 217, 0.96), rgba(12, 172, 240, 0.96)) !important;color:#f1f1f1;width:125px;height:33px;text-align:center;padding:7px 10px 7px 3px;}


#estatus_creado_group span{font-size:1.4em;}
#estatus_rojo_group span{font-size:1.4em;}
#estatus_purpura_group span{font-size:1.4em;}
#estatus_amarillo_group span{font-size:1.4em;}
#estatus_verde_group span{font-size:1.4em;}
#estatus_default_group span{font-size:1.4em;}
...
<div id="alerta_asignacion_minus" style="background:#f1f1f1;width:500px;display:none">
	<form action="<?=$this->config->base_url()?>index.php/director/quitar_asignacion" method="post" id="form_quitar_asignacion">
...
<div style='display:none;background:rgba(0,0,0,0.5);' id='eliminar_core'>
	<div style='margin:auto;margin-top:7px;width:200px;background:#f1f1f1;padding:21px;'>
		<form action="<?=$this->config->base_url()?>index.php/director/del_cor" method="post">
...
<div id="editar_core" style="background:rgba(0, 0, 0, 0.5);display:none;">
	...
	<form action="<?=$this->config->base_url()?>index.php/director/editar_core" method="post">
